...
def index(request,pIndex=1):
    ...
    ulist= umod.filter(status__lt = 9,company_id = wuser['company_id']) # 9 means delete so dun show 
    mywhere=[]
    #get search and find 
...
def add(request):
    ''' to add info'''
    return render (request,"web/account/add.html")
...
def insert(request):
    ...
    try:
        ob = Account()
        wuser=request.session.get('webuser',None)
        ob.company_id=wuser['company_id']
        ob.company_name=wuser['company_name']
            #print(wuser)
...
def edit(request,uid=0):
    ...
    try:
        ob = Account.objects.get(id=uid)
        context={'user':ob}
        return render(request,"web/account/edit.html",context)
    except Exception as err:
