...
def upload(request):
    return render(request,"mobile/upload.html")
...
def doupload(request):
    ...
    for cover_pic in piclist:
        ...
        if cover_pic.lower().endswith(('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tiff')):
            ...
            wuser=request.session.get('mobileuser',None)
            #print(wuser)
            op.user_id=wuser['id']
    ...
    return render(request, "mobile/showpic.html", {"piclist": piclist , "allpic":allpic})
