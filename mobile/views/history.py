...
def index (request,pIndex=1):
    ...
    if kw1:
        if kw:
            ...
            mywhere.append("from_keyword="+kw1)
            # mywhere.append("keyword="+end_str)
    ...
    return render(request,"mobile/history.html",{"piclist":list2,'plist':plist,'pIndex':pIndex,'maxpages':maxpages,'mywhere':mywhere})
...
def delete(request,pid=0):
    ...
    return redirect('/mobile/history/1')
...
def label(request,pid=0):
    ...
    #try:
        ob = Picture.objects.get(id=pid)
        context={'obj':ob}
        return render(request,"mobile/label.html",context)
